...
data "azuread_group" "platform_ops" {
  display_name = "Platform Operations SC"
}

# Filter out production management groups
locals {
  mg_non_prod = {
    for k, mg in var.management_groups :
    k => {
      id           = k
...
# Add Platform Operations SC as member
resource "azuread_group_member" "platform_ops_in_non_prod_contributors" {
  for_each         = var.enable_platform_ops_non_prod_membership ? data.azuread_group.dts_contributors_mg : {}
