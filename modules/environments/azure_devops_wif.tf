resource "azuredevops_serviceendpoint_azurerm" "endpoint_wif" {
  ...
  service_endpoint_name                  = "OPS-APPROVAL-GATE-${upper(var.env)}-ENVS-wif"
...
resource "azuread_application_federated_identity_credential" "endpoint_wif" {
  ...
  display_name   = replace(replace("OPS-APPROVAL-GATE-${upper(var.env)}-ENVS-wif", "/", "__"), ":", "_")
  audiences      = ["api://AzureADTokenExchange"]
  issuer         = azuredevops_serviceendpoint_azurerm.endpoint_wif[0].workload_identity_federation_issuer
  subject        = azuredevops_serviceendpoint_azurerm.endpoint_wif[0].workload_identity_federation_subject
}
