locals {
  ...
  groups = merge(
    ...
    # Add Contributor Eligible group for non-sandbox subscriptions (no RBAC assigned)
    !(lower(azurerm_subscription.this.subscription_name) == "sandbox" || lower(azurerm_subscription.this.subscription_name) == "sbox") ? {
